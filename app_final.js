...
window.deleteAporte = async function(id) {
     if (confirm('Â¿EstÃ¡ seguro de eliminar este aporte?')) {
...
function exportReporteCompletoExcel(filters, format = 'excel') {
    ...
    // Contributions sheet
    const contribData = [['Reporte de Aportes'], [], filterInfo, [], ['Fecha', 'Usuario', 'Monto (â‚¡)']];
    if (aportes.length > 0) {
        aportes.forEach(a => {
    ...
    // Expenses sheet
    const gastosData = [['Reporte de Gastos'], [], filterInfo, [], ['Fecha', 'RazÃ³n', 'Monto (â‚¡)']];
    if (gastos.length > 0) {
        gastos.forEach(g => {
...
function exportReporteCompletoPDF(filters) {
    ...
    if (aportes.length > 0) {
        ...
        doc.setTextColor(5, 150, 105);
        doc.setFont('helvetica', 'bold');
        doc.text(`TOTAL APORTES: â‚¡${totalAportes.toLocaleString('es-CR')}`, 16, yPos);
    } else {
    ...
    if (gastos.length > 0) {
        ...
        doc.setTextColor(220, 38, 38);
        doc.setFont('helvetica', 'bold');
        doc.text(`TOTAL GASTOS: â‚¡${totalGastos.toLocaleString('es-CR')}`, 16, yPos);
    } else {
    ...
    doc.setFont('helvetica', 'normal');
    usuariosAportaron.forEach(u => {
        doc.text(`â€¢ ${u.nombre} ${u.primerApellido} ${u.segundoApellido || ''}`, 18, yPos);
        yPos += 6;
    });
    yPos += 4;
    ...
    doc.setTextColor(0);
    doc.setFont('helvetica', 'normal');
    doc.text(`Total Aportes: â‚¡${totalAportesSum.toLocaleString('es-CR')}`, 14, yPos);
    yPos += 7;
    doc.text(`Total Gastos: â‚¡${totalGastosSum.toLocaleString('es-CR')}`, 14, yPos);
    yPos += 7;
    doc.setTextColor(sobrante >= 0 ? 5 : 220, sobrante >= 0 ? 150 : 38, sobrante >= 0 ? 105 : 38);
    doc.setFont('helvetica', 'bold');
    doc.text(`Sobrante: â‚¡${sobrante.toLocaleString('es-CR')}`, 14, yPos);
    ...
    for (let i = 1; i <= pageCount; i++) {
        doc.setPage(i);
        doc.text('DiseÃ±ado y programado por: Â©Tec.Alberto Hidalgo', 105, 287, { align: 'center' });
    }
...
function renderAporteHistory() {
    ...
    tbody.innerHTML = aportes.map((aporte, index) => `
        <tr style="animation: slideInUp 0.3s ease-out ${index * 0.05}s both;">
            ...
            <td>â‚¡ ${aporte.monto.toLocaleString('es-CR')}</td>
            <td>
...
function renderGastoHistory() {
    ...
    tbody.innerHTML = gastos.map((gasto, index) => `
        <tr style="animation: slideInUp 0.3s ease-out ${index * 0.05}s both;">
            ...
            <td>â‚¡ ${gasto.monto.toLocaleString('es-CR')}</td>
            <td>
...
function openAporteModal(aporte = null) {
    ...
    if (aporte) {
        ...
        document.getElementById('aporte-edit-usuario').value = aporte.usuarioId || '';
...
// ============= REPORTES DE GASTOS =============
// Las funciones de reportes de gastos (mes y rango) estÃ¡n implementadas
// en las lÃ­neas siguientes con sus correspondientes modales en el HTML.

// ============= TOTAL EN CAJA SECTION =============
// ============= LIMPIEZA DE HISTORIALES =============
document.getElementById('clear-aporte-history').addEventListener('click', function() {
    if (confirm('Â¿EstÃ¡ seguro de limpiar todo el historial de aportes?')) {
        DB.clearAportes();
        renderAporteHistory();
        updateTotalEnCaja();
...
document.getElementById('clear-gasto-history').addEventListener('click', function() {
    if (confirm('Â¿EstÃ¡ seguro de limpiar todo el historial de gastos?')) {
        DB.clearGastos();
        renderGastoHistory();
        updateTotalEnCaja();
